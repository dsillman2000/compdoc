...
from compdoc.exceptions import FormatUnsupportedDocException
...
class BaseFormatter(abc.ABC):
    ...
    @abc.abstractmethod
    def _format_class_doc(self, class_doc: ClassDoc) -> str: 
        ...
    ...
    def format_doc(self, doc: DocType) -> str:
        if isinstance(doc, ClassDoc):
            return self._format_class_doc(doc)
        elif isinstance(doc, FuncDoc):
            return self._format_func_doc(doc)
        else:
            raise FormatUnsupportedDocException('Unsupported doc type %s provided to formatter %s' % \
                                                (type(doc), self.__class__))


class DavidFormatter(BaseFormatter):
